...
(def jquery "function lookupEntry() {
            $.ajax({
	url: '/entry/' + $('#lookupName').val(),
	success: function(data) {
        $('#lookupEntryResult').text(JSON.stringify(data))
        },
	error: function(jqXHR, textStatus, errorThrown) { $('#lookupEntryResult').text(textStatus + \" \" + errorThrown); }
        })
	}
        ...
        function addEntry() {
            $.ajax({
                url: '/entry',
                data: JSON.stringify({_id: $('#addName').val(), email: $('#addEmail').val()}),
